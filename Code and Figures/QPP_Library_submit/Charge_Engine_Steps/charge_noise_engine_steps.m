%--------------------------------------%
% Charge Noise Engine - Steps
%--------------------------------------%
% Adapted from charge_noise_engine_init_basis_setup in tandem with
% calc_noisy_tetron_ev_steps.
%
% When running the previous time evolution: between time steps j and j+1, I
% would use the Hamiltonian defined at H(j+1) to carry out the evolution.
% This was fine for step time steps (delta_t), but is annoying for when I
% want to do large chemical potential change at large
% delta_t, which I want to use in our second paper. 
%
% Here I instead evolve using H(j) (from steps j to j+1). And calculate all
% quantities at "j+1" using the basis of H(j+1).
%
...
function [omega_0_from_fit, T2_from_fit, omega_0_guess, X_exp_mean,...
    ...
    charge_noise_engine_inst_basis_setup(mu_mean, mu_offset, w, delta, N, BC, t_init, t_final,...
    ...
    if isscalar(delta_t)
        ...
        if t_vec(end) ~= t_final
            error('delta_t vector does not match t_init and t_final');
        end
    end
    ...
    if strcmp(plot_results, 'plot_all_results') | strcmp(save_output, 'save_output')
        if length(delta_t) >1
            error('I have not implemented clean simulation with delta_t as a vector, add this code');
        end
        % Clean sim:
