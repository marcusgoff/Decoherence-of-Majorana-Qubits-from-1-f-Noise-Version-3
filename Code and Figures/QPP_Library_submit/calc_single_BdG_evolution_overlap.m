%-----------------------------%
% Calc Single BdG Evolution Overlap
%-----------------------------%
%
...
function [overlaps, psi_mat] = calc_single_BdG_evolution_overlap(mu_init,...
    mu_offset, w, delta, N, BC, t_init, t_final, delta_t, noise_traj)
    ...
    % mu_vec = mu_vec + mu_offset.*[+1; -1];
    
    mu_vec = mu_init + noise_traj.' + mu_offset.*[+1; -1];
    psi_mat = zeros(4*N, length(t_vec));
    psi_mat(:,1) = init_bdg_state;
    
    disp('Error somewhere in here, that isnt in ramp only')
