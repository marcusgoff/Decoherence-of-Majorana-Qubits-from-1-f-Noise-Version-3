...
function [omega_0_from_fit, T2_from_fit, omega_0_guess, X_exp_mean,...
         P_y1y2y3y4_mean, overlap_with_init_state_mean, overlap_fixed_basis_states_mean, ...
         t_vec, top_power_mat, bot_power_mat, mzm_parity_decay] = ...
    charge_noise_engine_general(mu_mean, mu_offset, w, delta, N, BC, t_init, t_final,...
     delta_t, noise_type, noise_parameters, num_trials, time_evolution_method,...
     save_output, plot_results, init_condition, basis_choice)  
    ...
    P_y1y2y3y4_mat = zeros(num_trials, length(t_vec));
    ...
    for trial_no = 1:num_trials
        ...
        P_y1y2y3y4_mat(trial_no, :) = P_y1y2y3y4;
    ...
    P_y1y2y3y4_mean = mean(P_y1y2y3y4_mat,1);
    ...
    %weights = linspace(1,0,length(t_vec)).^2;
    weights = ones(size(t_vec));
    log_mzm_parity = log(P_y1y2y3y4_mean);
