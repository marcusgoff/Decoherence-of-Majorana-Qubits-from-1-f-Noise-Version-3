...
% Same with Z_BdG. 
%
% Returns the tetron X operator as BdG matrix, defined as
...
%          after chain-1 chain-2 ordering.
%
% OUTPUTS:
...
function [X_BdG, X_BdG_QP] = get_tetron_X_operator_BdG(e_vecs, e_vals)

    check_inputs(e_vecs, e_vals);
    N = length(e_vals)./4;
    
    A = zeros(2*N); 
    A(N:N+1,N:N+1) = (1/4)*ones(2); 
    % Define X_BdG in quasiparticle dirac basis (without -2i factor):
    % ordered  by eigenvalues
    gamma_1_gamma_3_BdG_QP = [zeros(2*N), A; -A, zeros(2*N)];
    % Rotate to site basis and multiply by 2i:
    X_BdG =  - 2i*e_vecs*gamma_1_gamma_3_BdG_QP*e_vecs'; 
    X_BdG_QP = -2i*gamma_1_gamma_3_BdG_QP;
