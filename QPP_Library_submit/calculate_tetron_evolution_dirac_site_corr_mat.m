...
function [Z_exp, X_exp, Y_exp, t_vec, first_site_occ] = calculate_tetron_evolution_dirac_site_corr_mat(...
    corr_mat_init, e_vecs_init, e_vals_init, H_evolve, delta_t, t_init, t_final)

    check_inputs(e_vecs_init, e_vals_init)
 
    %U_delta_t_expm = expm(1i*H_evolve.*delta_t);  
   ...
   % [V, D_evolve] = eig(H_e);
    ...
    %[~, temp] = eig(U_delta_t_expm);
    %H_anti = (H_evolve - H_evolve')/2;
    %temp2 = exp(1i*diag(D_evolve).*delta_t);
...
%     U_delta_t = V*diag(exp(1i*diag(D_evolve)*delta_t))*V';    
    ...
    corr_mat_curr = corr_mat_init; % In site basis
    cov_mat_qp_curr = convert_corr_site_to_cov_qp(corr_mat_curr, e_vecs_init);   
    Z_exp(1) = cov_mat_qp_curr(1,1+N);
    X_exp(1) = cov_mat_qp_curr(1,1+2*N);
    Y_exp(1) = -cov_mat_qp_curr(1,1+3*N);   
    first_site_occ(1) = corr_mat_curr(1,1);
...
function cov_mat_qp = convert_corr_site_to_cov_qp(corr_mat_site, e_vecs)
    ...
    % Get QP Corr Matrix
    corr_mat_qp = e_vecs.'*corr_mat_site*conj(e_vecs);
