...
%-------------------------------------------------%
% FOUND AN UNRESOLVED BUG: breaking degeneracies between two chains results
% in an ordering of the eigenvectors which doesn't exactly correspond to
% the ordering of the eigenvalues. You can see this by comparing
% diag(e_vals) with e_vecs'*H_tetron*e_vecs. RESOLVE THIS. 
% ^RESPONSE: It's partly possible this is because the I mix states above and
% below E_BdG = 0 to obtain the maximally localised majoranas. 
% AH, no I was right before. I expect this will only happen at particularly 
% small energies, when the inbuilt "eig" function fails to distinguish 
% between the states properly. However, even for larger energies
% process_majorana_zero_modes_kitaev_chain 
% may switch the positive energy and negative energy states around. 
% 27_06_24 - I think I fixed the source of this issue in
% process_majorana_zero_modes_kitaev chain - I switched the columns of the
% deloc_dirac_zero_mode around. 
%
%
% Given two Kiteav chain BdG Hamiltonians, return the eigenvectors and
...
%       
% TO DO: add handling of majorana_zero_modes case where only one chain or 
% no chains are topological. 
%        Add Input checks. 

function [e_vecs, e_vals, majorana_zero_modes] = ... 
    diagonalise_uncoupled_tetron_via_Kitaev_chains(H_1, H_2, zero_mode_type)
    
    if mod(size(H_1,2),2) || mod(size(H_1,1),2) || ...
         size(H_1,1) ~= size(H_1,2)
        error('H_1 must be a 2N x 2N matrix');
    end
    if mod(size(H_2,2),2) || mod(size(H_2,1),2) || ...
         size(H_2,1) ~= size(H_2,2)
    ...
    % To do:
    % - Add handling of zero_mode_type == majorana_zero_modes case.
    
    % DEBUG
   % H = [H_1, zeros(size(H_1));zeros(size(H_1)), H_2];
   % norm(H - e_vecs*diag(e_vals)*e_vecs') %I think it's okay if this isn't
   % zero.
end
