...
function [gap_eff, mid_env_cell, top_env_cell, bot_env_cell, x_cell]  = ...
    get_effective_gap_inst_adiabatic_mid_envelope_25(n_chosen,...
    ...
    % Use 'fit'
    fit_eqn = '2*x + a'; %'-2*x + a'; Force slope of two for sake of getting the effective gap
    init_guess = 0.5;
   ...
   if plot_calcs
        fig1 = figure(); 
        loglog((x_mat./gap_1_min_vec.^2), y_mat);
        xlabel('v/g_1^2');
        ylabel('Leakage');
        axis tight;
    end
    ...
    for ii = 1:length(ramp_height_vec)
        ...
        % adiabatic_regime_up_bound is defined in terms of
        % ramp_rate_mat./gap_1_min_vec(ii).^2
        log_x_ind_ii = log10(x_mat(:, ii)) <= adiabatic_regime_up_bound + log10(gap_1_min_vec(ii).^2);
        ...
        if plot_calcs
            ...
            figure(); 
            subplot(1,2,1);
            plot(x./gap_1_min_vec(ii).^2, y); hold on;
            plot(min_mat(:,1)./gap_1_min_vec(ii).^2, min_mat(:,2), 'o'); 
            plot(max_mat(:,1)./gap_1_min_vec(ii).^2, max_mat(:,2), 'x'); 
            plot(x./gap_1_min_vec(ii).^2, y_mid, '-', 'LineWidth', 1.2);
            plot(x./gap_1_min_vec(ii).^2, top_envelope, '-.');
            plot(x./gap_1_min_vec(ii).^2, bot_envelope, '-.');  
            loglog(x./gap_1_min_vec(ii).^2, 10.^(f1.a).*x.^2, 'r-.', 'LineWidth', 1.2);
            xlabel('v/g_1^2');
            title(title_str); 
     
            subplot(1,2,2);            
            loglog(x./gap_1_min_vec(ii).^2, y); hold on;
            loglog(min_mat(:,1)./gap_1_min_vec(ii).^2, min_mat(:,2), 'o'); 
            loglog(max_mat(:,1)./gap_1_min_vec(ii).^2, max_mat(:,2), 'x'); 
            loglog(x./gap_1_min_vec(ii).^2, y_mid, '-', 'LineWidth', 1.2); 
            plot(x./gap_1_min_vec(ii).^2, top_envelope, '-.'); 
            loglog(x./gap_1_min_vec(ii).^2, bot_envelope, '-.');   
            loglog(x./gap_1_min_vec(ii).^2, 10.^(f1.a).*x.^2, 'r-.', 'LineWidth', 1.2);
            xlabel('v/g_1^2');
            title(title_str);         
        end
    ...
    if plot_calcs
        ...
        % figure();
        % ii = 1; loglog(x_mat(:, ii)./gap_eff(ii).^2, y_mat(:,ii)); hold on;
        % ii = 2; loglog(x_mat(:, ii)./gap_eff(ii).^2, y_mat(:,ii)); hold on;
        % ii = 3; loglog(x_mat(:, ii)./gap_eff(ii).^2, y_mat(:,ii)); hold on; 
        % xlabel('v/g_{eff}^2');
    end


    % Oh it's clearly because gap_eff is defined with respect to
    % x_mat = ramp_rate_mat./g_1.^2
    % DEBUG - DELETE
    % figure();
    % ii = 1; loglog(ramp_rate_mat(:, ii)./gap_eff(ii).^2, y_mat(:,ii)); hold on;
    % ii = 2; loglog(ramp_rate_mat(:, ii)./gap_eff(ii).^2, y_mat(:,ii)); hold on;
    % ii = 3; loglog(ramp_rate_mat(:, ii)./gap_eff(ii).^2, y_mat(:,ii)); hold on;
end
